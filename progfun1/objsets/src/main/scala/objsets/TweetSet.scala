...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = (left, right) match {
    ...
    case (Empty, _)     => if (elem.retweets >= right.mostRetweeted.retweets) elem else right.mostRetweeted
    case (_, Empty)     => if (elem.retweets >= left.mostRetweeted.retweets) elem else left.mostRetweeted
  }
