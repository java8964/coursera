...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] = neighbors filter { case (b, m) => ! (explored contains b) }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (b, ms) #:: rest => {
      val more = newNeighborsOnly(neighborsWithHistory(b, ms), explored + b)
      more ++ from(rest ++ more, explored + b)
    }
  }
