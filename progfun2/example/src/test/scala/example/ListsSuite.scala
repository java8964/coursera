 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of list with negative numbers") {
  ...
  test("Max one element list should return this one value") {
    assert(max(List(3)) === 3)
  }
  
  test("Max list with negative numbers") {
    assert(max(List(2, -2, 1)) === 2)
  }

  test("Max list with duplicate numbers") {
    assert(max(List(2, -2, 2, 1, -2)) === 2)
  }
