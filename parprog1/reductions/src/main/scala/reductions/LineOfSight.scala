...
object LineOfSight {
  ...
  def calcAngle(index: Int, height: Float): Float = 
    if (index == 0) 0f else height / index
    
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    var currMax = 0f
    for (i <- input.indices) {
      currMax = max(currMax, calcAngle(i, input(i)))
      output(i) = currMax
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    while (idx < until) {
      currMax = max(currMax, calcAngle(idx, input(idx)))
      idx += 1
    }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end - from <= threshold)
      ...
      val (left, right) =
        parallel(
          upsweep(input, from, from + (end - from) / 2, threshold),
          upsweep(input, from + (end - from) / 2, end, threshold)
        )
      Node(left, right)  
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    var idx = from
    var curMaxAngle = startingAngle
    while (idx < until) {
      curMaxAngle = max(curMaxAngle, calcAngle(idx, input(idx)))
      output(idx) = curMaxAngle
      idx += 1
    }
  }
