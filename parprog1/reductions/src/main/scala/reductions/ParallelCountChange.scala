...
object ParallelCountChange {
  ...
  def countChange(money: Int, coins: List[Int]): Int = (money, coins) match {
    ...
    case (_, Nil) => 0
    case (m, _) if m < 0 => 0
    case (m, head :: Nil) if (m == head) => 1
    case rest => countChange(money - coins.head, coins) + countChange(money, coins.tail)
  }
