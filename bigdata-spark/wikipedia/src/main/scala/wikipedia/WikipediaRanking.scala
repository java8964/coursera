...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local[4]").setAppName("wikipedia").set("driver-memory", "4g")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = 
    rdd.filter(wa => wa.text.split(" ").contains(lang)).count().toInt
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(
      ws => {
        for (word <- ws.text.split(" ").distinct if langs.contains(word)) yield (word, ws)
      }
    ).groupByKey()
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(
      ws => {
        for (word <- ws.text.split(" ").distinct if langs.contains(word)) yield (word, 1)
      }
